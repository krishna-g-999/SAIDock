...
# DEMO_MODE: True on Streamlit Cloud (no Vina binary, no HPC home dir)
DEMO_MODE = (
    not Path('/home/krishnasalini-rs').exists()
    and not Path('/usr/local/bin/vina').exists()
    and not Path('/opt/vina').exists()
...
<style>
  ...
  .demo-banner { background: #EFF6FF;
                 border-left: 4px solid #0D2137;
                 padding: 0.8rem 1.2rem;
                 border-radius: 0 6px 6px 0;
                 font-size: 0.9rem; color: #0D2137;
                 margin-bottom: 1rem; }
</style>
...
if not run_btn:
    ...
    with c3:
        if st.button("Ellagic Acid vs CDK6 (5L2I)",
                     use_container_width=True):
            st.session_state['ql'] = 'OC1=C2C(=CC(=C1)O)C1=C(OC2=O)C(=O)OC2=CC(=CC(=C21)O)O'
    ...
    try:
        ...
        resp   = requests.post(f'{API_URL}/submit', json=payload, timeout=10)
        job    = resp.json()
        job_id = job['job_id']
        status_msg.success(f"Job submitted. ID: {job_id}")
        _run_mode = 'api'
    except Exception:
        status_msg.warning("API server not reachable. Running directly in this session.")
        _run_mode = 'local'

    if _run_mode == 'api':
        while True:
            try:
                stat = requests.get(f'{API_URL}/status/{job_id}', timeout=10).json()
            except Exception:
                time.sleep(5)
                continue
            ...
            if stat['status'] == 'COMPLETE':
                ...
                st.link_button("Download Full HTML Report",
...
def _show_results(state: dict):
    ...
    dtss_color = color_map.get(cat, '#555')

    st.markdown(f"""
    <div class="dtss-card" style="background:{dtss_color}; margin-bottom:1rem">
      DTSS: {dtss:.3f} &nbsp;|&nbsp; {cat} Binding
    </div>
    """, unsafe_allow_html=True)

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("DTSS Score", f"{dtss:.3f}")
    with col2:
        st.metric("Best Vina Score", f"{best:.2f} kcal/mol")
    with col3:
